...
putFrame tree frame =
    tree { frames = Map.insert (Frame.frId frame) frame (frames tree) }
...
addFrame tree parent =
    let
        frame = Frame.empty (frameCount tree) (Frame.frId parent)
...
lookupLexically fn tree frame name =
    case Map.lookup name (fn frame) of
      Nothing | Frame.frId frame == Frame.frParentId frame -> Nothing
              | otherwise -> let Just parent = getFrame tree (Frame.frParentId frame) in
...
getLexicalSymbol = lookupLexically Frame.frSymbols
...
addSymbol tree frame name sym =
  let frame' = frame { frSymbols = Map.insert name sym (Frame.frSymbols frame) } in
  putFrame tree frame'
