...
import Data.SrcFile
import qualified Data.SrcFile as SrcFile (exprs, symbols, renNs)
import Data.Stx
...
interpretInteractive fs srcfile@SrcFile { t = InteractiveT, deps, renNs = Just ns } =
    let
        (exprs, env) = interpretNamespace interactiveFs deps interactiveNs
        env' = envToExprs srcfile env
    in
     (srcfile { exprs = Map.union (SrcFile.exprs srcfile) env' }, last exprs)
    where interactiveNs =
            let  SrcFile { renNs = Just (Namespace uses stxs) } = srcfile in
            Namespace (uses ++ [("Interactive", "")]) stxs

          interactiveFs =
            Map.insert "Interactive" srcfile { renNs = Just interactiveNs } fs